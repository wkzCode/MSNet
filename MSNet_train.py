...
logging.basicConfig(
    filename=save_path + "RGBD.log",
    format="[%(asctime)s-%(filename)s-%(levelname)s:%(message)s]",
    level=logging.INFO,
    filemode="a",
    datefmt="%Y-%m-%d %I:%M:%S %p",
)
logging.info("MSNet-Train_4_pairs")
    ...
    try:
        for i, (images, gts, depth, edge) in enumerate(train_loader, start=1):
            ...
            if i % 100 == 0 or i == total_step or i == 1:
                print(
                    "{} Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], LR:{:.7f}||sal_loss:{:4f} ||edge_loss:{:4f} ||sal_loss1:{:4f}".format(
                        datetime.now(),
                        epoch,
                        opt.epoch,
                        i,
                ...
                logging.info(
                    "#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], LR:{:.7f},  sal_loss:{:4f} ||edge_loss:{:4f}||sal_loss1:{:4f} , mem_use:{:.0f}MB".format(
                        epoch,
                        opt.epoch,
                        i,
                        total_step,
        ...
        if (epoch) % 5 == 0:
            torch.save(
                model.state_dict(), save_path + "MSNet_epoch_{}.pth".format(epoch)
            )
        ...
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        torch.save(
            model.state_dict(), save_path + "MSNet_epoch_{}.pth".format(epoch + 1)
        )
    ...
    with torch.no_grad():
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), save_path + "MSNet_epoch_best.pth")
